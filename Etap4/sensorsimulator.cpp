...
void SensorSimulator::generate_temp(double previous_temp1, double previous_temp2,
                                        double previous_temp3, double previous_temp4, unsigned int how_many)
{
    ...
    for (unsigned int b = 0; b < how_many; b++)
    {
    ...
    for (unsigned int c = 0; c < how_many; c++)
    {
    ...
    for (unsigned int d = 0; d < how_many; d++)
    {
    ...
    if(temps1.size() == how_many)
        temps1.erase(temps1.begin());

    if(temps2.size() == how_many)
        temps2.erase(temps2.begin());

    if(temps3.size() == how_many)
        temps3.erase(temps3.begin());

    if(temps4.size() == how_many)
